...
# ============================================================================
# МАТЕМАТИЧЕСКОЕ ЯДРО (MCM И ПРОГНОЗ)
# ============================================================================

def calculate_mcm(md: List[float], inc: List[float], azi: List[float]) -> pd.DataFrame:
    """Расчет координат и DLS методом минимальной кривизны (API RP 7G)"""
    n = len(md)
    ...
    for i in range(1, n):
        ...
        cos_alpha = max(-1.0, min(1.0, cos_alpha)) # Защита от float ошибок
    ...
    return pd.DataFrame({
        "MD": md, "INC": inc, "AZI": azi, "TVD": tvd, "NORTH": north, "EAST": east, "DLS": dls
    })

def predict_next_point(current_md, current_inc, current_azi, k_slide, k_rotary, 
                       slide_pct, tool_face, formation_anisotropy, rheology_mod, length):
    """Предиктивный расчет следующей точки на основе модели увода"""
    slide_frac = slide_pct / 100.0
    
    # Моделирование приращения угла (упрощенная физическая модель)
    ...
    # Моделирование приращения азимута
...
def create_trajectory_layout():
    return html.Div([
        html.Div([
            ...
            html.P("Прогноз пространственного положения, аудит коммерческих рисков и адаптация КНБК", 
        ...
        dbc.Row([
            dbc.Col([
                html.Label("Заказчик (для лимитов ТК):", style={"fontWeight": "bold", "fontSize": "13px"}),
                dcc.Dropdown(id='traj-client', options=[
                    {"label": "Роснефть", "value": "Роснефть"}, {"label": "Газпром нефть", "value": "Газпром нефть"},
                    {"label": "ЛУКОЙЛ", "value": "ЛУКОЙЛ"}, {"label": "Татнефть", "value": "Татнефть"}, {"label": "Прочие", "value": "Прочие"}
                ], value="Роснефть", clearable=False)
            ], width=3),
            dbc.Col([
                html.Label("Текущая глубина MD, м:", style={"fontWeight": "bold", "fontSize": "13px"}),
                dcc.Input(id='traj-curr-md', type='number', value=2500.0, style={"width": "100%", "padding": "8px"})
            ], width=3),
            dbc.Col([
                html.Label("Текущий зенит (INC), °:", style={"fontWeight": "bold", "fontSize": "13px"}),
                dcc.Input(id='traj-curr-inc', type='number', value=45.0, style={"width": "100%", "padding": "8px"})
            ], width=3),
            dbc.Col([
                html.Label("Текущий азимут (AZI), °:", style={"fontWeight": "bold", "fontSize": "13px"}),
                dcc.Input(id='traj-curr-azi', type='number', value=120.0, style={"width": "100%", "padding": "8px"})
            ], width=3),
        ], className="mb-4"),
        ...
        html.Div(id='traj-tabs-content', style={"marginTop": "20px"}),
        
        # Хранилище для истории рекомендаций
        dcc.Store(id='traj-journal-store', data=[]),
    ])

def create_forecast_tab():
    return html.Div([
        html.H4("Параметры прогнозного интервала", style={"color": "#0F172A", "marginBottom": "15px"}),
        dbc.Row([
            dbc.Col([
                html.Label("Длина интервала, м:", style={"fontWeight": "bold"}),
                dcc.Input(id='traj-length', type='number', value=30.0, min=10, max=100, style={"width": "100%", "padding": "8px"})
            ], width=3),
            dbc.Col([
                html.Label("Доля слайда, %:", style={"fontWeight": "bold"}),
                dcc.Input(id='traj-slide-pct', type='number', value=40.0, min=0, max=100, style={"width": "100%", "padding": "8px"})
            ], width=3),
            dbc.Col([
                html.Label("Tool Face, °:", style={"fontWeight": "bold"}),
                dcc.Input(id='traj-tf', type='number', value=45.0, min=0, max=360, style={"width": "100%", "padding": "8px"})
            ], width=3),
            dbc.Col([
                html.Label("Анизотропия пласта:", style={"fontWeight": "bold"}),
                dcc.Input(id='traj-aniso', type='number', value=0.05, min=0, max=0.2, step=0.01, style={"width": "100%", "padding": "8px"})
            ], width=3),
        ], className="mb-4"),
        ...
        dbc.Row([
            dbc.Col([
                html.Div(className="metric-card", children=[
                    html.Div(className="metric-label", children="ПРОГНОЗ MD"),
                    html.Div(id='traj-res-md', className="metric-value", children="0.0 м")
                ])
            ], width=3),
            dbc.Col([
                html.Div(className="metric-card", children=[
                    html.Div(className="metric-label", children="ПРОГНОЗ INC"),
                    html.Div(id='traj-res-inc', className="metric-value", children="0.0 °")
                ])
            ], width=3),
            dbc.Col([
                html.Div(className="metric-card", children=[
                    html.Div(className="metric-label", children="ПРОГНОЗ AZI"),
                    html.Div(id='traj-res-azi', className="metric-value", children="0.0 °")
                ])
            ], width=3),
            dbc.Col([
                html.Div(className="metric-card", children=[
                    html.Div(className="metric-label", children="ПРОГНОЗ DLS"),
                    html.Div(id='traj-res-dls', className="metric-value", children="0.0 °/10м")
                ])
            ], width=3),
        ], className="mb-4"),
        
        html.H4("Профиль траектории (План vs Прогноз)", style={"color": "#0F172A", "marginBottom": "10px"}),
        dcc.Graph(id='traj-profile-chart', style={"height": "500px"}),
    ])

def create_risks_tab():
    return html.Div([
        html.H4("Аудит коммерческих рисков по DLS", style={"color": "#0F172A", "marginBottom": "15px"}),
        dbc.Row([
            dbc.Col([
                html.Label("Макс. лимит DLS по ТК, °/10м:", style={"fontWeight": "bold"}),
                dcc.Input(id='traj-dls-max', type='number', value=3.0, step=0.1, style={"width": "100%", "padding": "8px"})
            ], width=4),
            dbc.Col([
                html.Label("Мин. набор DLS по ТК, °/10м:", style={"fontWeight": "bold"}),
                dcc.Input(id='traj-dls-min', type='number', value=0.5, step=0.1, style={"width": "100%", "padding": "8px"})
            ], width=4),
            dbc.Col([
                html.Label("Последние 2 фактических замера DLS:", style={"fontWeight": "bold"}),
                dbc.InputGroup([
                    dbc.Input(id='traj-dls-hist-1', type='number', value=2.8, step=0.1),
                    dbc.Input(id='traj-dls-hist-2', type='number', value=2.9, step=0.1),
                ])
            ], width=4),
        ], className="mb-4"),
        ...
        html.H5("Рекомендация по режиму бурения на интервал:", style={"color": "#0F172A", "marginBottom": "10px"}),
        dbc.Row([
            dbc.Col([
                html.Div(className="metric-card", children=[
                    html.Div(className="metric-label", children="РЕКОМЕНДУЕМЫЙ СЛАЙД"),
                    html.Div(id='traj-rec-slide', className="metric-value", children="0.0 м")
                ])
            ], width=6),
            dbc.Col([
                html.Div(className="metric-card", children=[
                    html.Div(className="metric-label", children="РЕКОМЕНДУЕМЫЙ РОТОР"),
                    html.Div(id='traj-rec-rotary', className="metric-value", children="0.0 м")
                ])
            ], width=6),
        ]),
    ])
...
def create_learning_tab():
    return html.Div([
        html.H4("Адаптивное самообучение коэффициентов КНБК", style={"color": "#0F172A", "marginBottom": "15px"}),
        ...
        dbc.Row([
            dbc.Col([
                html.Label("Факт. длина интервала, м:", style={"fontWeight": "bold"}),
                dcc.Input(id='learn-length', type='number', value=30.0, style={"width": "100%", "padding": "8px"})
            ], width=4),
            dbc.Col([
                html.Label("Факт. набранный угол (INC), °:", style={"fontWeight": "bold"}),
                dcc.Input(id='learn-inc', type='number', value=1.2, step=0.1, style={"width": "100%", "padding": "8px"})
            ], width=4),
            dbc.Col([
                html.Label("Факт. доля слайда, %:", style={"fontWeight": "bold"}),
                dcc.Input(id='learn-slide-pct', type='number', value=40.0, style={"width": "100%", "padding": "8px"})
            ], width=4),
        ], className="mb-4"),
        ...
        html.H5("Журнал рекомендаций рейса", style={"color": "#0F172A", "marginBottom": "10px"}),
...
def trajectory_forecast_callbacks(app, data_bridge):
    ...
    @app.callback(Output('traj-tabs-content', 'children'), Input('traj-tabs', 'value'))
    def render_traj_tab(tab):
        if tab == 'tab-forecast': return create_forecast_tab()
        if tab == 'tab-risks': return create_risks_tab()
        if tab == 'tab-learning': return create_learning_tab()
        return html.Div()

    # 1. Подтягивание лимитов из Compliance Engine
    @app.callback(
    ...
    def calculate_forecast(md, inc, azi, length, slide_pct, tf, aniso):
        ...
        # Базовые коэффициенты (в реальности брать из data_bridge или настроек КНБК)
        k_slide = data_bridge.get_data('k_slide', 0.45)
        k_rotary = data_bridge.get_data('k_rotary', 0.02)
        rheo_mod = 1.0 # Упрощено для примера
        
        next_md, next_inc, next_azi, forecast_dls = predict_next_point(
            md, inc, azi, k_slide, k_rotary, slide_pct, tf, aniso, rheo_mod, length
        )
        
        # График
        fig = go.Figure()
        # Плановая линия (условно прямая для примера)
        fig.add_trace(go.Scatter(x=[md, next_md], y=[inc, inc + (forecast_dls/10)*length], 
                                 name="Прогноз", line=dict(color="#3B82F6", width=3, dash="dot")))
        # Лимиты
        fig.add_trace(go.Scatter(x=[md, next_md], y=[3.0, 3.0], name="Лимит DLS", line=dict(color="#EF4444", width=2)))
        
        fig.update_layout(
            title="Прогноз изменения зенитного угла",
            xaxis_title="Глубина MD, м", yaxis_title="Зенитный угол INC, °",
            template="plotly_white", height=500
        )
        
        return (f"{next_md:.1f} м", f"{next_inc:.2f} °", f"{next_azi:.2f} °", 
                f"{forecast_dls:.2f} °/10м", fig)
    ...
    def check_penalties(dls_max, dls_min, hist1, hist2, forecast_dls_str):
        ...
        # Упрощенная рекомендация (в реальности решать обратную задачу от целевого INC)
        rec_slide = 30.0 if forecast_dls < dls_min else 10.0
        rec_rotary = 30.0 - rec_slide
        
        return alert, f"{rec_slide:.1f} м", f"{rec_rotary:.1f} м"

    # 4. Самообучение (Исправленная логика без NameError)
    @app.callback(
    ...
    def adapt_k_factors(n_clicks, curr_md, curr_inc, curr_azi, length, actual_inc, slide_pct):
        ...
        # Теоретический прогноз по старой модели
    ...
    # 5. Журнал (упрощенный Store)
    @app.callback(
        Output('traj-journal-table', 'children'),
        Input('traj-journal-store', 'data')
    )
    def update_journal(data):
        if not data:
            return html.P("Журнал пуст", className="text-muted")
