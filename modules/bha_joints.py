...
def calculate_joints_table():
    ...
    for i in range(len(bha_elements_for_joints) - 1):
        ...
        if delta_d > CRITICAL_DELTA_D:
            ...
            status = "в допуске"
        joints_data.append({
            "Стык №": i + 1, "Элемент А": el_top["name"], "S/N А": el_top["sn"],
            "Элемент Б": el_bot["name"], "S/N Б": el_bot["sn"],
            "OD А (мм)": el_top["od"], "OD Б (мм)": el_bot["od"],
            "ΔD (мм)": round(delta_d, 1), "Допуск СТО": f"{CRITICAL_DELTA_D:.1f} мм", "Статус": status
        })
...
def get_joints_table_component():
    ...
    column_defs = [
        ...
        {"headerName": "Элемент А", "field": "Элемент А", "width": 200},
        {"headerName": "Элемент Б", "field": "Элемент Б", "width": 200},
    ...
    row_style = {
        "styleConditions": [
            {"condition": "params.data['Статус'] === 'КРИТИЧЕСКИЙ ПЕРЕПАД'", "style": {"backgroundColor": "#FEE2E2", "color": "#991B1B"}},
            {"condition": "params.data['Статус'] === 'Повышенный перепад'", "style": {"backgroundColor": "#FEF3C7", "color": "#92400E"}},
            {"condition": "params.data['Статус'] === 'в допуске'", "style": {"backgroundColor": "#D1FAE5", "color": "#065F46"}},
        ],
    ...
    if not critical_joints.empty:
        alternatives_section = html.Div([
            html.H5("Рекомендации по устранению критических стыков:", style={"color": "#dc2626", "marginTop": "20px", "marginBottom": "15px"}),
            *[create_alternative_suggestion({'idx': row['Стык №'], 'top': row['Элемент А'], 'bot': row['Элемент Б'], 'delta': row['ΔD (мм)'], 'od_top': row['OD А (мм)'], 'od_bot': row['OD Б (мм)']}) for _, row in critical_joints.iterrows()]
        ])
    else:
        alternatives_section = html.Div("")
    return html.Div([
        dag.AgGrid(id='joints-table', rowData=df_joints.to_dict('records'), columnDefs=column_defs, defaultColDef={"resizable": True, "sortable": True, "filter": True}, style={"height": "300px", "width": "100%"}, dashGridOptions={"rowHeight": 35, "headerHeight": 40, "domLayout": "normal", "rowStyle": row_style}),
