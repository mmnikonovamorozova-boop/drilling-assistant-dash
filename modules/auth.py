...
def auth_layout():
    return html.Div([
        ...
        html.Div([
            html.H2("Вход в систему", className="text-center mb-4", style={"color": "#1E40AF", "fontWeight": "700"}),
                    ...
                    dbc.Form([
                        dbc.Row([
                            dbc.Col([
                                ...
                                dbc.Input(id='auth-username', type='text', placeholder='user или admin', className="mb-3", value="user")
                            ], width=12),
                        ]),
                        dbc.Row([
                            dbc.Col([
                                dbc.Label("Пароль", style={"fontWeight": "bold"}),
                                dbc.Input(id='auth-password', type='password', placeholder='Введите пароль', className="mb-3", value="1234")
                            ], width=12),
                        ]),
                        dbc.Row([
                            dbc.Col([
                                dbc.Button("Войти в систему", id='auth-login-btn', color="primary", className="w-100", size="lg", style={"fontWeight": "bold", "marginTop": "10px"})
                            ], width=12),
                        ]),
...
def auth_callbacks(app, data_bridge):
    @app.callback(
        Output('auth-redirect-url', 'pathname'),
