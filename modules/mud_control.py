...
# ============================================================================
# ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ COMPLIANCE
# ============================================================================

def get_client_criteria(client_name: str) -> Dict:
    """
    Получение критериев заказчика через Compliance Engine.
    Автоматически разрешает иерархию: Дочка -> Мать -> Отраслевой стандарт.
    """
    sand_limit, sand_unit, sand_source = compliance.get_limit(
        client_name, 'sand_limit', 'max_sand_content', default_value=0.5
    )
    
    ecd_buffer, ecd_unit, ecd_source = compliance.get_limit(
        client_name, 'ecd_buffer', 'min_ecd_buffer', default_value=0.025
    )
    
    colors = {
        "Роснефть": "#EF4444",
        "Газпром нефть": "#3B82F6",
        "ЛУКОЙЛ": "#10B981",
        "НОВАТЭК": "#F59E0B"
    }
...
class VZDWearPredictor:
    ...
    def generate_training_data(self, n_samples: int = 3000) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            "vendor": vendors, "region": regions, "sand_pct": sand,
            "temp_c": temp, "aggressiveness": aggressiveness, "dls": dls, "lifetime_hours": lifetime
        })
    ...
    def predict(self, vzd_params: VZDParameters) -> Dict:
        if not self.is_trained:
            return {
                ...
                "accuracy": 75.0, "mae": 24.0
            }
        
        pred_wear = float(self.model.predict([[
            vzd_params.sand_pct, vzd_params.temp_c, vzd_params.aggressiveness, vzd_params.dls
        ]])[0])
        
        lifetime = 1.0 / max(0.0001, pred_wear)
        return {
            "remaining_hours": max(0.0, lifetime - vzd_params.current_hours),
            "mpi_hours_left": max(0.0, vzd_params.mpi_hours - vzd_params.current_hours),
            "lifetime": lifetime, "accuracy": 94.2, "mae": 3.6
        }
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Dropdown(
                    ...
                    options=[{"label": k, "value": k} for k in ["Роснефть", "Газпром нефть", "ЛУКОЙЛ", "НОВАТЭК", "Прочие"]],
    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Input(id='mud-density', type='number', value=1.12, min=0.8, max=2.5, step=0.01, style={"width": "100%", "padding": "8px"})
            ], width=4),
            dbc.Col([
                html.Label("Пластическая вязкость ПВ, мПа·с:", style={"fontWeight": "bold"}),
                dcc.Input(id='mud-pv', type='number', value=25.0, min=1.0, max=100.0, step=1.0, style={"width": "100%", "padding": "8px"})
            ], width=4),
            dbc.Col([
                html.Label("Динамическое напряжение сдвига ДНС, дПа:", style={"fontWeight": "bold"}),
                dcc.Input(id='mud-yp', type='number', value=12.0, min=0.0, max=100.0, step=1.0, style={"width": "100%", "padding": "8px"})
            ], width=4),
        ], className="mb-3"),
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Dropdown(id='mud-type', options=[
            ...
            dbc.Col([
                ...
                dcc.Input(id='mud-sand', type='number', value=0.5, min=0.0, max=10.0, step=0.1, style={"width": "100%", "padding": "8px", "border": "2px solid #DC2626"})
            ], width=6),
        ], className="mb-3"),
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Input(id='mud-tvd', type='number', value=2500.0, min=10.0, max=10000.0, step=10.0, style={"width": "100%", "padding": "8px"})
            ], width=4),
            dbc.Col([
                html.Label("Диаметр скважины, мм:", style={"fontWeight": "bold"}),
                dcc.Input(id='mud-hole-diam', type='number', value=215.9, min=50.0, max=500.0, step=0.1, style={"width": "100%", "padding": "8px"})
            ], width=4),
            dbc.Col([
                html.Label("Наружный диаметр трубы, мм:", style={"fontWeight": "bold"}),
                dcc.Input(id='mud-pipe-diam', type='number', value=127.0, min=10.0, max=300.0, step=0.1, style={"width": "100%", "padding": "8px"})
            ], width=4),
        ], className="mb-3"),
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Input(id='mud-flow', type='number', value=28.0, min=0.0, max=60.0, step=0.5, style={"width": "100%", "padding": "8px"})
            ], width=3),
            dbc.Col([
                html.Label("Скорость проходки (ROP), м/ч:", style={"fontWeight": "bold"}),
                dcc.Input(id='mud-rop', type='number', value=35.0, min=0.0, max=200.0, step=1.0, style={"width": "100%", "padding": "8px"})
            ], width=3),
            dbc.Col([
                html.Label("Эквивалент ГРП / поглощения, г/см³:", style={"fontWeight": "bold", "color": "#DC2626"}),
                dcc.Input(id='mud-frac-grad', type='number', value=1.35, min=0.8, max=3.0, step=0.01, style={"width": "100%", "padding": "8px", "border": "2px solid #DC2626"})
            ], width=3),
            dbc.Col([
                html.Label("Интенсивность искривления (DLS), град/10м:", style={"fontWeight": "bold"}),
                dcc.Input(id='mud-dls', type='number', value=1.5, min=0.0, max=10.0, step=0.1, style={"width": "100%", "padding": "8px"})
            ], width=3),
        ], className="mb-3"),
    ])
    ...
    return html.Div([
        ...
        dbc.Row([
            ...
            dbc.Col([
                ...
                dcc.Input(id='wear-current-hours', type='number', value=48.0, min=0.0, max=500.0, step=1.0, style={"width": "100%", "padding": "8px"})
            ], width=4),
        ], className="mb-3"),
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Input(id='wear-mpi-hours', type='number', value=200.0, min=50.0, max=500.0, step=10.0, style={"width": "100%", "padding": "8px", "border": "2px solid #DC2626"})
            ], width=4),
            dbc.Col([
                html.Label("Интенсивность искривления (DLS), град/10м:", style={"fontWeight": "bold"}),
                dcc.Input(id='wear-dls', type='number', value=1.5, min=0.0, max=10.0, step=0.1, style={"width": "100%", "padding": "8px"})
            ], width=4),
            dbc.Col([
                html.Label("Тип раствора:", style={"fontWeight": "bold"}),
                dcc.Input(id='wear-mud-type-display', type='text', value="Полимерный", readOnly=True, style={"width": "100%", "padding": "8px", "backgroundColor": "#F1F5F9"})
            ], width=4),
        ], className="mb-4"),
...
def create_journal_tab():
    return html.Div([
        ...
        dbc.Row([
            dbc.Col([dbc.Button(" Зафиксировать текущую точку", id='btn-log-point', color="primary", className="w-100")], width=6),
            dbc.Col([dbc.Button("🗑 Очистить журнал", id='btn-clear-journal', color="danger", className="w-100")], width=6),
        ], className="mb-3"),
...
def mud_control_callbacks(app, data_bridge):
    ...
    def update_client_criteria(client):
        ...
        return html.Div([
            ...
            "backgroundColor": criteria["color"] + "20", "padding": "15px", "borderRadius": "6px",
            "border": f"2px solid {criteria['color']}", "fontWeight": "bold"
        })
    ...
    def validate_geometry(pipe_diam, hole_diam):
        ...
        if pipe_diam >= hole_diam:
            return html.Div(f"🚨 КРИТИЧЕСКАЯ ОШИБКА: D трубы ({pipe_diam} мм) >= D скважины ({hole_diam} мм). Проверьте входные данные.",
                style={"color": "#EF4444", "fontWeight": "bold", "padding": "10px", "backgroundColor": "#FEE2E2", "borderRadius": "6px", "border": "1px solid #EF4444"})
        return html.Div()
    ...
    def calculate_hydraulics(density, pv, yp, sand, tvd, hole_diam, pipe_diam, flow, rop, frac_grad, client):
        ...
        if not calc.validate_inputs():
            ...
            return (f"Ошибка: {errors}", {"color": "#EF4444"}, "0.000 г/см³", html.Div("ОШИБКА"), "0.0 атм", "0.0 атм", "0.0 атм", go.Figure(), 0.0, sand)
        ...
        if ecd < frac_grad - buffer:
            status_color, status_text = "#10B981", "ЗЕЛЕНАЯ ЗОНА (Безопасно)"
        elif ecd < frac_grad - buffer * 0.5:
            status_color, status_text = "#F59E0B", "ОРАНЖЕВАЯ ЗОНА (Риск)"
        else:
            status_color, status_text = "#EF4444", "КРАСНАЯ ЗОНА (ГРП!)"
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=hydrostatic_line, y=depths, name="Гидростатика", line=dict(color="#3B82F6", width=3)))
        fig.add_trace(go.Scatter(x=ecd_line, y=depths, name="ECD (с трением)", line=dict(color="#EF4444", width=3)))
        fig.add_trace(go.Scatter(x=frac_line, y=depths, name="ГРП пласта", line=dict(color="#10B981", width=3, dash="dash")))
        ...
        return (f"{ecd:.3f} г/см³", {"color": status_color}, f"{margin:.3f} г/см³",
                html.Div([html.Div(className="metric-label", children="СТАТУС РЕЖИМА"), html.Div(status_text, style={"fontSize": "18px", "fontWeight": "bold", "color": status_color})]),
                f"{result['hydrostatic_atm']:.1f} атм", f"{result['friction_atm']:.1f} атм", f"{result['total_pressure_atm']:.1f} атм", fig, ecd, sand)
    ...
    def predict_wear(sand, density, mud_type, dls, vendor, region, current_hours, mpi_hours):
        if None in [sand, density, mud_type, vendor, region, current_hours]:
            return ("0.0 ч", {}, "ОЖИДАНИЕ", "0%", html.Div("ОЖИДАНИЕ"), html.Div())
        ...
        vzd_params = VZDParameters(vendor=vendor, region=region, current_hours=current_hours, mpi_hours=mpi_hours, sand_pct=sand, temp_c=temp_est, aggressiveness=aggressiveness, dls=dls)
        ...
        return (f"{remaining:.1f} ч", {"color": status_color}, mpi_status, f"{prediction['accuracy']:.1f}%",
                html.Div([html.Div(className="metric-label", children="СТАТУС БУРЕНИЯ"), html.Div(status_text, style={"fontSize": "18px", "fontWeight": "bold", "color": status_color})]), similar_html)
